...
function Output($name='',$dest='')
{
	...
	if ($this->showStats) {
		file_put_contents(FP_PDF_SAVE_LOCATION . 'mPDF_performance_log.txt', date('Y-m-d h:i:s') . ' Generated in '.sprintf('%.2F',(microtime(true) - $this->time0))." seconds\r\n", FILE_APPEND);
	}
	...
	if (count($this->PDFAXwarnings) && (($this->PDFA && !$this->PDFAauto) || ($this->PDFX && !$this->PDFXauto))) {
		if ($this->PDFA) {
			...
			echo '<div>These issues can be automatically fixed by setting <i>"pdfa1b_force" => true</i> in the Formidable Pro PDF Extended configuration.php file</div>';
			...
			echo '<div>These issues can be automatically fixed by setting <i>"pdfx1a_force" => true</i> in the Formidable Pro PDF Extended configuration.php file</div>';
	...
	if ($this->showStats) {
			...
			file_put_contents(FP_PDF_SAVE_LOCATION . 'mPDF_performance_log.txt', $log, FILE_APPEND);
	}
